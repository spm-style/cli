...
class Use {
	constructor(instances, options, path) {
		...
		this.usePathes = options.path === true ? [] : options.path;
		this.path = path;
...
//     if(name == null){ // this case is install without name
//       this.name = null;
//       this.version = null;
//     }else{ // this case is install with name or names and with or without version
//       let tmp = name.split("@");
//       if(tmp.length == 2){
//         this.name = tmp[0];
//         this.version = tmp[1];
