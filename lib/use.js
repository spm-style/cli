...
let Separator = require('inquirer').Separator
    ...
    if (use.usePathes && use.usePathes.length) {
      for (let item of use.usePathes) {
        if (item.endsWith('.css') || item.endsWith('.scss')) {
          if (Fs.existsSync(`${item.startsWith(CONST.SEPARATOR) ? item : use.initialPath + CONST.SEPARATOR + item}`)) { return resolve(use) }
        }
      }
...
let findPackagePromise = (use) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (Fs.existsSync(`${use.path}${CONST.SEPARATOR}package-spm.json`)) {
      use.pathPackage = `${use.path}${CONST.SEPARATOR}package-spm.json`
      if (!Fs.existsSync(`${use.path}${CONST.SEPARATOR}spm_modules`)) {
        return reject(new Error('please install a module before using it'))
      } else {
        use.pathModules = `${use.path}${CONST.SEPARATOR}spm_modules`
      }
      ...
      useBis.path = use.path.substring(0, use.path.lastIndexOf(CONST.SEPARATOR))
...
let selectModulePromise = (use) => {
  ...
  return new Promise((resolve, reject) => {
    Fs.readdir(`${use.pathModules}`, (err, files) => {
      ...
      Common.updateRegistryModulesPromise(use)
      .then(() => {
...
let selectClassPromise = (use) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    for (let module of use.modules) {
      promises.push(Common.getPackageSpmFilePromise(`${use.pathModules}${CONST.SEPARATOR}${module}${CONST.SEPARATOR}package-spm.json`)
      .then(res => {
        let jsonFile = res
        if (!jsonFile) {
...
let selectInstancePromise = (use) => {
  ...
  return new Promise((resolve, reject) => {
    let promises = []
    for (let module of use.modules) {
      if (Fs.existsSync(`${use.pathModules}${CONST.SEPARATOR}${module}${CONST.SEPARATOR}dist`)) {
        promises.push(listInstancesInDistPromise(`${use.pathModules}${CONST.SEPARATOR}${module}${CONST.SEPARATOR}dist`, module))
      }
    }
    Promise.all(promises)
    .then(res => {
      ...
      if (tmpInstances.length) {
        ...
        for (let instance in multiInstance) {
          if (multiInstance[instance].length === 1 && use.instances.includes(instance.substring(0, instance.lastIndexOf('.')))) {
            use.pathInstances[`${use.pathModules}${CONST.SEPARATOR}${multiInstance[instance][0].module}${CONST.SEPARATOR}dist${CONST.SEPARATOR}${multiInstance[instance][0].instance}`] = multiInstance[instance][0]
          } else {
        ...
        if (choices.length === 1) {
          use.pathInstances = {}
          use.pathInstances[`${use.pathModules}${CONST.SEPARATOR}${instanceMap[choices[0]].module}${CONST.SEPARATOR}dist${CONST.SEPARATOR}${instanceMap[choices[0]].instance}`] = instanceMap[choices[0]]
          return resolve(use)
          ...
          .then(answer => {
            for (let item of answer.pathInstances) {
              use.pathInstances[`${use.pathModules}${CONST.SEPARATOR}${instanceMap[item].module}${CONST.SEPARATOR}dist${CONST.SEPARATOR}${instanceMap[item].instance}`] = instanceMap[item]
            }
            return resolve(use)
          })
          .catch(reject)
      ...
      for (let file of files) {
        if (file.endsWith('.scss') || file.endsWith('.css')) {
          ...
          if (use.jsonFile.entry &&
            Fs.existsSync(`${use.path}${CONST.SEPARATOR}${use.jsonFile.entry}`) &&
            Path.relative(`${filePath}${CONST.SEPARATOR}${file}`, `${filePath}${CONST.SEPARATOR}${use.jsonFile.entry}`) === '') { isEntry = true }
          subObject[file] = isEntry ? 'entry' : 'regular'
          flag = true
        }
        if (Fs.statSync(`${filePath}${CONST.SEPARATOR}${file}`).isDirectory() &&
          Path.relative(`${filePath}${CONST.SEPARATOR}${file}`, use.pathModules) !== '' &&
          file !== 'node_modules') {
          subObject[file] = {}
          promises.push(recursiveListPathesPromise(subObject[file], `${filePath}${CONST.SEPARATOR}${file}`, use))
        }
      }
      Promise.all(promises)
      .then(booleans => {
...
let recursiveFilesTree = (obj, level, prefix, choices, choiceMap, path) => {
  ...
  for (let file in obj) {
    if (obj[file] === 'regular' || obj[file] === 'entry') {
      ...
      choiceMap[`${Chalk.hex('#717171')(prefix + '|  ')}${fullFile}`] = {full: `${path}${CONST.SEPARATOR}${file}`, short: file}
    } else if (obj[file] !== null && typeof obj[file] === 'object' && Object.keys(obj[file]).length) { folders.push(file) }
  }
  for (let i = 0; i < folders.length; i++) {
    choices.push(new Separator(`  ${prefix}|_ ${folders[i]}${CONST.SEPARATOR}`))
    recursiveFilesTree(obj[folders[i]], level + 1, prefix + (i !== folders.length - 1 ? '|  ' : '   '), choices, choiceMap, `${path}${CONST.SEPARATOR}${folders[i]}`)
  }
}
...
let createFilesChoicesPromise = (use) => {
  ...
  return new Promise((resolve, reject) => {
    use.choiceMap = {}
    let choices = [new Separator(use.path)]
...
let selectTargetFilesPromise = (use) => {
  ...
  return new Promise((resolve, reject) => {
    if (!use.usePathes && use.jsonFile.entry) {
      ...
      use.usePathes = {}
      use.usePathes[`${use.path}${CONST.SEPARATOR}${use.jsonFile.entry}`] = use.jsonFile.entry
      return resolve(use)
    } else if (use.usePathes.length) {
      // cas avec un liste d'argument
      let tmpUsePathes = {}
      for (let item of use.usePathes) {
        if (item.startsWith(`.${CONST.SEPARATOR}`)) {
          tmpUsePathes[`${use.initialPath}${CONST.SEPARATOR}${item.substring(2)}`] = item.substring(2)
        } else if (item.startsWith(`..${CONST.SEPARATOR}`)) {
          tmpUsePathes[`${use.initialPath}${CONST.SEPARATOR}${item}`] = item
        } else {
...
let detectModulePackage = (module, use) => {
  if (use.debug) { Debug() }
  return new Promise((resolve, reject) => {
    Common.getPackageSpmFilePromise(`${use.pathModules}${CONST.SEPARATOR}${module}${CONST.SEPARATOR}package-spm.json`)
    .then(res => {
      if (res === null) { return reject(new Error(`module ${module} has no package-spm.json - reinstall using -f option`)) }
      return resolve(res)
    })
    .catch(reject)
  })
...
let detectInstanceParameters = (instance, use) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    Fs.readFile(`${use.pathModules}${CONST.SEPARATOR}${content.module}${CONST.SEPARATOR}${use.modulesMap[content.module].entry}`, 'utf8', (err1, data1) => {
      if (err1 && err1 !== 'ENOENT') {
        return reject(err1)
      } else if (err1) {
      ...
      Fs.readFile(`${use.pathModules}${CONST.SEPARATOR}${content.module}${CONST.SEPARATOR}dist${CONST.SEPARATOR}${content.instance.endsWith('.css') ? 'src' + CONST.SEPARATOR : ''}${content.instance.substring(0, content.instance.lastIndexOf('.'))}.scss`, 'utf8', (err2, data2) => {
        if (err2 && err2 !== 'ENOENT') {
          return reject(err2)
        } else if (err2) {
