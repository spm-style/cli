...
let listInstancesInDistPromise = (distPath, module) => {
  return new Promise((resolve, reject) => {
    Fs.readdir(distPath, (err, files) => {
      ...
      let instances = []
      for (let file of files) {
        if (file.endsWith('.scss')) { instances.push({module, instance: file.substring(0, file.length - 5)}) }
      }
      ...
      if (tmpInstances.length) {
        ...
        for (let instance in multiInstance) {
          if (multiInstance[instance].length === 1 && use.instances.includes(instance)) {
            use.pathInstances[`${use.pathModules}/${multiInstance[instance][0].module}/dist/${multiInstance[instance][0].instance}.scss`] = multiInstance[instance][0]
          } else {
