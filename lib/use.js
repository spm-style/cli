...
let detectInstanceParameters = (instance, use) => {
  return new Promise((resolve, reject) => {
    ...
    Fs.readFile(`${use.pathModules}/${content.module}/${use.modulesMap[content.module].entry}`, 'utf8', (err1, data1) => {
      if (err1 && err1 !== 'ENOENT') { return reject(err1) }
      else if (err1) { return reject(new Error(`couldn't save ${content.instance} from module ${content.module} - missing entry file`)) }
      ...
      Fs.readFile(`${use.pathModules}/${content.module}/dist/${content.instance.endsWith('.css') ? 'src/' : ''}${content.instance.substring(0, content.instance.lastIndexOf('.'))}.scss`, 'utf8', (err2, data2) => {
        if (err2 && err2 !== 'ENOENT') { return reject(err2) }
        else if (err2) { return reject(new Error(`couldn't save ${content.instance} from module ${content.module} - missing scss src file`)) }
