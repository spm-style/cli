  ...
  return new Promise((resolve, reject) => {
    ...
    while (currentDirectory !== CONST.USER_DIRECTORY) {
      if (Fs.existsSync(`${currentDirectory}${CONST.SEPARATOR}package-spm.json`)) {
        install.pathPackage = `${currentDirectory}${CONST.SEPARATOR}package-spm.json`
    ...
    Fs.mkdir(CONST.SPM_DIRECTORY, err => {
      ...
      Fs.mkdir(CONST.REGISTRY_PATH, err => {
        ...
        if (!install.pathProject) {
          let currentDirectory = Common.getCurrentPath()
          while (!Fs.existsSync(`${currentDirectory}${CONST.SEPARATOR}spm_modules`) && currentDirectory !== CONST.USER_DIRECTORY) {
            currentDirectory = currentDirectory.substring(0, currentDirectory.lastIndexOf(CONST.SEPARATOR))
          }
          if (currentDirectory === CONST.USER_DIRECTORY && Fs.existsSync(`${currentDirectory}${CONST.SEPARATOR}spm_modules`)) {
            ...
            install.warnings.push(`installing in folder: ${currentDirectory}${CONST.SEPARATOR}spm_modules`)
            install.pathProject = currentDirectory
          } else {
            return reject(new Error(CONST.ERROR.OUT_OF_SCOPE))
          }
        }
        install.current.path = `${install.pathProject}${CONST.SEPARATOR}spm_modules`
        install.pathModules = install.isRegistry ? install.pathRegistry : `${install.pathProject}${CONST.SEPARATOR}spm_modules`
        return resolve(install)
      })
    })
  })
...
let isInRegistry = (install) => {
  if (install.debug) { Debug() }
  let registryPath = `${CONST.REGISTRY_PATH}${CONST.SEPARATOR}${install.current.name}${CONST.SEPARATOR}${install.current.version}`
  if (Common.directoryExists(registryPath)) {
    install.current.target = `${CONST.REGISTRY_PATH}${CONST.SEPARATOR}${install.current.name}${CONST.SEPARATOR}${install.current.version}`
    return true
  }
  if (install.isRegistry) { install.current.target = `${CONST.REGISTRY_PATH}${CONST.SEPARATOR}${install.current.name}${CONST.SEPARATOR}${install.current.version}` }
...
let isInProject = (install) => {
  ...
  while (!install.current.secondLevel && Path.relative(currentDirectory, install.pathProject).startsWith('..')) {
    let jsonFile = Common.getPackageSpmFileSync(`${currentDirectory}${CONST.SEPARATOR}${install.current.name}${CONST.SEPARATOR}package-spm.json`)
    if (jsonFile) {
      if (jsonFile.version === install.current.version) {
        install.current.target = `${currentDirectory}${CONST.SEPARATOR}${install.current.name}`
        return true
      } else if (previousPath) {
        install.current.bestPath = install.current.bestPath || `${previousPath}${CONST.SEPARATOR}spm_modules`
      } else {
  ...
  install.current.bestPath = install.current.bestPath || `${install.pathProject}${CONST.SEPARATOR}spm_modules`
  return false
}
  ...
  switch (type) {
      ...
      for (let item of install.downloadList) {
        if (item.name === install.current.name) {
          if (item.version === install.current.version) {
            install.current.target = `${item.path}${CONST.SEPARATOR}spm_modules${CONST.SEPARATOR}${install.current.name}`
            ...
            return true
          } else if (install.current.bestPath && !Common.unrealRelativePath(install.current.bestPath, install.current.path.split(CONST.SEPARATOR).slice(0, item.path.split(CONST.SEPARATOR).length + 1).join(CONST.SEPARATOR)).startsWith(`..${CONST.SEPARATOR}..${CONST.SEPARATOR}..${CONST.SEPARATOR}`)) {
            install.current.bestPath = install.current.path.split(CONST.SEPARATOR).slice(0, item.path.split(CONST.SEPARATOR).length + 1).join(CONST.SEPARATOR)
          }
        }
      }
      break
    ...
    case 'instance':
      for (let item of install.instanceList) {
        if (item.path === `${install.current.target}${CONST.SEPARATOR}dist` && item.name === install.current.instanceName) { return true }
      }
      break
  }
  if (install.current.bestPath) { install.current.target = `${install.current.bestPath}${CONST.SEPARATOR}${install.current.name}` }
...
let createDirectoriesSourcePromise = (install) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    while (Common.unrealRelativePath(currentDirectory, `${install.current.path}${CONST.SEPARATOR}${install.current.name}`) !== '') {
      ...
      currentDirectory = `${currentDirectory}${CONST.SEPARATOR}${Common.unrealRelativePath(currentDirectory, `${install.current.path}${CONST.SEPARATOR}${install.current.name}`).split(CONST.SEPARATOR)[0]}`
    }
    return resolve(install)
  })
}
...
let createDirectoriesTargetPath = (install) => {
  ...
  try {
    ...
    while (Common.unrealRelativePath(currentDirectory, `${install.current.target}${CONST.SEPARATOR}dist`) !== '') {
      if (!install.directoryList.includes(Common.cleanFilePath(currentDirectory)) && !Common.directoryExists(currentDirectory)) {
        install.directoryList.push(Common.cleanFilePath(currentDirectory))
        if (install.debug) { Debug('creating directory1', currentDirectory) }
        Fs.mkdirSync(currentDirectory)
      }
      currentDirectory = `${currentDirectory}${CONST.SEPARATOR}${Common.unrealRelativePath(currentDirectory, `${install.current.target}${CONST.SEPARATOR}dist`).split(CONST.SEPARATOR)[0]}`
    }
    return install
  } catch (err) {
...
let addMainClassesPromise = (current, install) => {
  ...
  return new Promise((resolve, reject) => {
    try {
      if (!Fs.existsSync(`${current.target}${CONST.SEPARATOR}${current.jsonFile.entry}`)) {
        return reject(new Error(`incorrect entry file in module ${current.name}@${current.version}`))
      }
      if (!Common.directoryExists(`${current.target}${CONST.SEPARATOR}dist`)) { Fs.mkdirSync(`${current.target}${CONST.SEPARATOR}dist`) }
      Fs.readFile(`${current.target}${CONST.SEPARATOR}${current.jsonFile.entry}`, 'utf8', (err, data) => {
        ...
        Fs.writeFile(`${current.target}${CONST.SEPARATOR}dist${CONST.SEPARATOR}${current.name}.scss`, input, err => {
          if (err) { return reject(err) }
  ...
  return new Promise((resolve, reject) => {
    ...
    if (!alreadyInList('symlink', install) &&
      ...
      if (install.current.enable) {
        if (`${install.current.path}${CONST.SEPARATOR}${install.current.name}` === install.current.target) {
          ...
          install.current.newPromise = install.current.newPromise.then(() => { return Common.createSymlinkPromise(`${install.current.path}${CONST.SEPARATOR}${install.current.name}`, install.current.target) })
          ...
          install.symlinkList.push({
            ...
            path: `${install.current.path}${CONST.SEPARATOR}${install.current.name}`,
    ...
    // otherwise, the dependency has to be added to the install list
      ...
      if (!install.current.target) { install.current.target = `${install.current.path}${CONST.SEPARATOR}${install.current.name}` }
      ...
      .then(() => Common.writeContent(JSON.stringify(install.current.jsonFile, null, '  ') + '\n', `${install.current.target}${CONST.SEPARATOR}package-spm.json`, '', install.current))
      .then(() => addMainClassesPromise(install.current, install))
      .then(() => {
        if (install.style === 'css') {
          return Common.convertScssToCss(install.current, `${install.current.target}${CONST.SEPARATOR}dist`, install.current.name)
        } else {
          return resolve(install.current)
        }
      })
      ...
      if (install.current.target && Common.unrealRelativePath(`${install.current.path}${CONST.SEPARATOR}${install.current.name}`, install.current.target) !== '') {
      ...
      if (install.current.instances) {
        for (let instance in install.current.instances) {
          if (install.current.target && !alreadyInList('instance', install) &&
            (install.isForce || !Fs.existsSync(`${install.current.target}${CONST.SEPARATOR}dist${CONST.SEPARATOR}${instance}.scss`))) {
            install.instanceList.push({
              path: `${install.current.target}${CONST.SEPARATOR}dist`,
              name: instance,
              variables: install.current.instances[instance]
            })
            if (!install.directoryList.includes(Common.cleanFilePath(`${install.current.target}${CONST.SEPARATOR}dist`)) && !Common.directoryExists(`${install.current.target}${CONST.SEPARATOR}dist`)) {
              install.directoryList.push(Common.cleanFilePath(`${install.current.target}${CONST.SEPARATOR}dist`))
              Common.createFolderIfUnexistantSync(`${install.current.target}${CONST.SEPARATOR}dist`)
            }
...
let loopForInRecursivePromise = (install, table, index = 0, promises = []) => {
  ...
  return new Promise((resolve, reject) => {
    if (index >= table.length) {
      ...
      installBis.current.path = install.current.parentPath ? `${install.current.target}${CONST.SEPARATOR}spm_modules` : install.current.path
      installBis.current.secondLevelPath = install.current.parentPath ? install.current.secondLevelPath || `${install.current.target}${CONST.SEPARATOR}spm_modules` : null
...
module.exports = (Program) => {
  return new Promise((resolve, reject) => {
    ...
    .option('-r, --registry', `to copy it in your ~${CONST.SEPARATOR}.spm${CONST.SEPARATOR}registry`)
