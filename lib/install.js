  ...
  return new Promise((resolve, reject) => {
    if(!install.isGlobal && !install.isRegistry){
      ...
      Fs.writeFile(`${CONST.SPM_PATH}/test.txt`, (err, data) => {
        if(err){
          return reject(CONST.ERROR.NO_SUDO_FOR_GLOBAL);
        }else{
...
let findPackageSpmPromise = (install) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (install.names.length > 0){
			...
			return reject(CONST.ERROR.NO_PACKAGE_SPM);
		}
  });
}
...
let getDependenciesInstall = (install) => {
  ...
  return new Promise((resolve, reject) => {
    if(install.pathPackage){
      ...
      if(install.names.length == 0){
        reject('if you want install add name of module or list in package-spm');
      }else{
        install.addDependenciesNames(install.names);
        return resolve(install);
      }
...
let isInRegistry = (install) => {
	...
	if (install.isRegistry)
		install.current.target = `${CONST.REGISTRY_PATH}/${install.current.name}/${install.current.version}`;
	return false; //vérifier qu'il y a un package et au moins un autre fichier root
}

let isInProject = (install) => {
	//recursive jusqu'au dossier principal pour voir si le fichier existe déjà ainsi que le meilleur path d'install
	let currentDirectory = install.current.path;
	let previousPath;
	while (
		(!install.current.secondLevel && Path.relative(currentDirectory, install.pathProject).startsWith('..'))
		// ||
		// (install.current.secondLevel && Path.relative(currentDirectory, install.current.secondLevel).startsWith('..'))
		){
	    ...
	    Request(url, (error, response, body) => {
	      	if (error){
	       		if(error.code == 'ECONNREFUSED')
	        		return reject('Server down check method getJsonApiPromise');
	       		else
	       			return reject(error);
			}else{
		...
		case 'symlink':
			for (let item of install.symlinkList){
				if (item.name == install.current.name && item.version == install.current.version &&
					(item.path == install.current.path && item.target == install.current.target))
					return true;
			}
			break;
		case 'instance':
			for (let item of install.instanceList){
				if (item.path == `${install.current.target}/dist` && item.name == install.current.instanceName)
					return true;
			}
			break;
	}
	if (install.current.bestPath)
		install.current.target = `${install.current.bestPath}/${install.current.name}`;
	return false;
}
	...
	return new Promise(async (resolve, reject) => {
		try {
			if (!Fs.existsSync(`${current.target}/${current.jsonFile.entry}`))
				return reject(`incorrect entry file in module ${current.name}@${current.version}`);
			if (!Common.directoryExists(`${current.target}/dist`))
				Fs.mkdirSync(`${current.target}/dist`);
...
let createListRecursivePromise = (install) => {
	return new Promise(async (resolve, reject) => {
		...
		//la dépendance existe déjà ou est en cours d'installation
		...
		//la dépendance n'existe pas, il faut l'installer
		}else{
			...
			//si on optimise le chemin d'installation de la dépendance, il faut faire un symlink vers le répertoire d'installation
		...
		if (!createDirectoriesPath(install))
			return reject(Chalk.hex(CONST.ERROR_COLOR)('error creating working directories'));
		if (debug) console.log('\n\n: == INSTALL AFTER == :\n\n', install.current, '\n\n ================= \n\n');
		if (install.current.instances){
			for (let instance in install.current.instances){
				// install.current.instanceName = instance;
...
loopForInRecursivePromise = (install, table, index = 0, promises = []) => {
...
let printArborescence = (settings) => {
	settings.maxLen += 6;
  	let i = 0, j;
  	let startIndex = 0;
  	let final = '';
  		...
  		if (line.instances && Object.keys(line.instances).length){
  			...
  			for (let instance in line.instances){
  				if (first){
  					...
  					if (indicator + 1 < settings.results.length && settings.results[indicator + 1].level)
  						final += `\n${Array(settings.results[indicator + 1].level * 3).join(' ')}${Chalk.hex('#F6EAB7')('|')}${Array(settings.maxLen - settings.results[indicator + 1].level * 3).join(' ')}`;
  					else
  	...
  	final += '\n';
	console.log('\nThe following has been installed in your project:\n', final);
}

let printArborescenceRecursive = (arborescence, settings, bases = [''], level = 0, res = '') => {
  let round = 1;
  let tmpRes;
  let result = res;
  for(let key of Object.keys(arborescence)){
    ...
    if(arborescence[key] != null){
      ...
      printArborescenceRecursive(arborescence[key].dependencies, settings, newBase, newLevel, result);
    }
    round++;
  }
  if (!level)
  	printArborescence(settings);
  return round;
};
...
let printArborescencePromise = (install) => {
  ...
  return new Promise((resolve, reject) => {
  	...
  	try{
    	printArborescenceRecursive(install.current.arborescence, arborescenceSettings);
  	}catch (err){
  		reject(err);
  	}
    return resolve(install);
