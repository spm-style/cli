...
let getDependenciesInstall = (install) => {
  ...
  return new Promise((resolve, reject) => {
    if (install.pathPackage) {
        ...
        if (!json) {
          ...
          if (install.names.length == 0) {
          	if (json.dependencies && typeof json.dependencies === 'object') {
	            ...
	            return resolve(install)
          	} else          		{ return reject(CONST.ERROR.NO_PACKAGE_TO_INSTALL) }
          } else {
      ...
      if (install.names.length == 0) { return reject(CONST.ERROR.NO_PACKAGE_TO_INSTALL) } else {
        install.addDependenciesNames(install.names)
        return resolve(install)
      }
    }
  ...
  return new Promise((resolve, reject) => {
    let url = `http://api.spm-style.com/packages/${install.current.name}`
	    ...
	    Request(url, (error, response, body) => {
	      	if (error) {
	       		if (error.code == 'ECONNREFUSED')	        		{ return reject('Server down check method getJsonApiPromise') }	       		else	       			{ return reject(error) }
      } else {
...
let addMainClassesPromise = (current) => {
  return new Promise((resolve, reject) => {
    try {
      if (!Fs.existsSync(`${current.target}/${current.jsonFile.entry}`)) { return reject(`incorrect entry file in module ${current.name}@${current.version}`) }
...
let createListRecursivePromise = (install) => {
  return new Promise((resolve, reject) => {
    ...
    if (!createDirectoriesPath(install)) { return reject(Chalk.hex(CONST.ERROR_COLOR)('error creating working directories')) }
...
let printArborescenceRecursive = (arborescence, settings, bases = [''], level = 0, res = '') => {
  ...
  if (!level)  	{ printArborescence(settings) }
  return round
}
  ...
  return new Promise((resolve, reject) => {
    Program
		...
		.action((names, options) => {
		    ...
		    .then(res => { return resolve(res) })
		    .catch(reject)
		    // .catch(err => {console.log(Chalk.hex(CONST.ERROR_COLOR)(err)); return reject(err)});
}).on('--help', function () {
