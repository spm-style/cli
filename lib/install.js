...
// let createLocalModulesDirectoryPromise = (install) => {
//   return new Promise((resolve, reject) => {
//     if (DisplayFuncName) { console.log('funct° - createLocalModulesDirectoryPromise') }
//     Fs.mkdir(install.pathModules, err => {
//       if (err && err.code !== 'EEXIST') { return reject(err) }
//       return resolve(install)
//     })
//   })
...
let isInRegistry = (install) => {
  if (DisplayFuncName) { console.log('funct° - isInRegistry') }
...
let isInProject = (install) => {
  if (DisplayFuncName) { console.log('funct° - isInProject') }
...
let getJsonPackageFromAPIPromise = (install) => {
  return new Promise((resolve, reject) => {
    if (DisplayFuncName) { console.log('funct° - getJsonPackageFromAPIPromise') }
    let url = `http://api.spm-style.com/package/install/${install.current.name}`
    if (install.current.version) { url += `?version=${install.current.version}` }
...
let alreadyInList = (type, install) => {
  if (DisplayFuncName) { console.log('funct° - alreadyInList') }
  switch (type) {
    case 'download':
...
let createDirectoriesSourcePromise = (install) => {
  if (debug) { console.log('<><><> createDirectoriesSourcePromise') }
  return new Promise((resolve, reject) => {
    let currentDirectory = install.pathProject
    while (Common.unrealRelativePath(currentDirectory, `${install.current.path}/${install.current.name}`) !== '') {
      if (!install.directoryList.includes(Common.cleanFilePath(currentDirectory)) && !Common.directoryExists(currentDirectory)) {
        ...
        console.log('creating directory2', currentDirectory)
...
let createDirectoriesTargetPath = (install) => {
  if (debug) { console.log('<><><> createDirectoriesTargetPath') }
  try {
    let currentDirectory = Common.unrealRelativePath(install.pathProject, install.current.target).startsWith('..') ? CONST.REGISTRY_PATH : install.pathProject
    while (Common.unrealRelativePath(currentDirectory, `${install.current.target}/dist`) !== '') {
      if (!install.directoryList.includes(Common.cleanFilePath(currentDirectory)) && !Common.directoryExists(currentDirectory)) {
        install.directoryList.push(Common.cleanFilePath(currentDirectory))
        console.log('creating directory1', currentDirectory)
        Fs.mkdirSync(currentDirectory)
      }
    ...
    console.log('error in createDirectoriesTargetPath\n', err)
    return null
  }
}
...
let addMainClassesPromise = (current, install) => {
  if (DisplayFuncName) { console.log('funct° - addMainClassesPromise') }
  return new Promise((resolve, reject) => {
    try {
      //A SUPPRIMER AVEC L'UPDATE DU CONTROLLER_PUBLISH => index.css to index.scss
      current.jsonFile.entry = `${current.jsonFile.entry.substring(0, current.jsonFile.entry.lastIndexOf('.'))}.scss`
...
let createListRecursivePromise = (install) => {
  return new Promise((resolve, reject) => {
    if (DisplayFuncName) { console.log('funct° - createListRecursivePromise') }
    ...
    if (!alreadyInList('symlink', install) &&
      ...
      if (`${install.current.path}/${install.current.name}` === install.current.target) {
        ...
        if (debug) console.log('=> on symlink', install.current.name, 'dans', install.current.path, 'vers', install.current.target)
    ...
    // otherwise, the dependency has to be added to the install list
      ...
      if (debug) console.log('=> on download in folder')
...
let loopForInRecursivePromise = (install, table, index = 0, promises = []) => {
  return new Promise((resolve, reject) => {
    ...
    if (index === table.length) { console.log('END OF PROMISES'); return resolve(promises) } else {
