...
/* synchronously returns a json file */
...
let downloadModuleSpmPromise = (name, version, targetPath) => {
  return new Promise((resolve, reject) => {
    ...
    if (!targetPath.startsWith(CONST.SEPARATOR)) { targetPath = `${__dirname}${separator}${targetPath}` }
...
let deleteFolderRecursive = (path, callback, firstRecursive = true) => {
  if (Fs.existsSync(path)) {
    for (let file of Fs.readdirSync(path)) {
      let currentPath = path + CONST.SEPARATOR + file
...
let cleanFilePath = (filePath) => {
  let tab = filePath.split(CONST.SEPARATOR)
  ...
  let res = tab.join(CONST.SEPARATOR)
  while (res.endsWith(CONST.SEPARATOR)) { res = res.slice(0, -1) }
  return res
}
...
let defineMixinName = (filePath) => {
  let table = filePath.split(CONST.SEPARATOR)
  let res = ''
  if (table.length < 1) {
...
let createInstancePromise = (name, target, entry, parameters, packageClasses) => {
  return new Promise((resolve, reject) => {
    parametersInstanceParsingPromise(name, `${target}${CONST.SEPARATOR}${entry}`, parameters, packageClasses)
    .then(parametersList => {
      let data = `@import '..${CONST.SEPARATOR}${entry}';\n\n@include spm-class_${parameters.target.name}(${parametersList});\n`
      Fs.writeFile(`{target}${CONST.SEPARATOR}dist${CONST.SEPARATOR}${name}.scss`, data, err => {
    ...
    Fs.readFile(item, 'utf8', (err, data) => {
      ...
      for (let pathInstance in useBis.pathInstances) {
        ...
        for (let importedFile of detectedImports) {
          if (Path.relative(`${item.substring(0, item.lastIndexOf(CONST.SEPARATOR))}/${importedFile}`, pathInstance) === '') {
...
let convertScssToCss = (item, path, file) => {
  return new Promise((resolve, reject) => {
    Sass.render({
      file: `${path}${CONST.SEPARATOR}${file}.scss`,
      outFile: `${path}${CONST.SEPARATOR}${file}.css`
    }, function (err, result) {
      if (err) { return reject(err) }
      Fs.writeFile(`${path}${CONST.SEPARATOR}${file}.css`, result.css, err => {
        if (err) { return reject(err) }
        Fs.rename(`${path}${CONST.SEPARATOR}${file}.scss`, `${path}${CONST.SEPARATOR}src${CONST.SEPARATOR}${file}.scss`, err => {
          if (err) { return reject(err) }
          return resolve(item)
        })
      })
...
let tgzFilePromise = (input, output) => {
  return new Promise((resolve, reject) => {
    ...
    while (input !== '' && input.endsWith(CONST.SEPARATOR)) { input = input.slice(0, -1) }
    while (output !== '' && output.endsWith(CONST.SEPARATOR)) { output = output.slice(0, -1) }
    if (!input.startsWith(CONST.SEPARATOR)) { input = `${__dirname}${CONST.SEPARATOR}${input}` }
    if (!output.startsWith(CONST.SEPARATOR)) { output = `${__dirname}${CONST.SEPARATOR}${output}` }
    let i = input.lastIndexOf(CONST.SEPARATOR)
    if (i === -1) {
      inputFolder = ''
      inputFile = input
    } else {
...
let createSymlinkLevelsPromise = (src, dest, level = 0) => {
  return new Promise((resolve, reject) => {
    if (src.split(CONST.SEPARATOR).length <= level + 2) {
      Fs.symlink(dest, src, err => {
        if (err) { return reject(err) }
        return resolve(src)
      })
    } else {
      Fs.mkdir(src.split(CONST.SEPARATOR).slice(0, 2 - src.split(CONST.SEPARATOR).length + level).join(CONST.SEPARATOR), err => {
        if (err && err.code !== 'EEXIST') { return reject(err) }
