...
let writeContent = (res, name, path = '', returnValue = false) => {
  return new Promise((resolve, reject) => {
    Fs.writeFile(path + name, res, 'utf8', err => {
      if (err) {
        return reject(new Error('Writing error or need to bu sudo'))
      }
...
let convertScssToCss = (item, path, file, remove = true) => {
  return new Promise((resolve, reject) => {
    Sass.render({
      file: `${path}/${file}.scss`,
      ...
      if (err) { return reject(err) }
      Fs.writeFile(`${path}/${file}.css`, result.css, error => {
        if (error) { return reject(error) }
        if (remove) {
          Fs.unlink(`${path}/${file}.scss`, err => {
            if (err) { return reject(err) }
            return resolve(item)
          })
        } else {
...
let displayMessagesPromise = (item) => {
  return new Promise((resolve, reject) => {
    if (item.errors.length) { console.log('') }
    for (let msg of item.errors) {
      console.log(`${Chalk.hex(CONST.ERROR_COLOR)('ERROR')}: ${msg}`)
    }
