...
let downloadModuleSpmPromise = (name, version, targetPath) => {
  console.log('>> DOWNLOAD-MODULE', name, version)
  return new Promise((resolve, reject) => {
    ...
    Fs.mkdir(targetPath, err => {
      ...
      .on('response', res => {
        if (res.statusCode >= 400) {
          console.log(Chalk.hex(CONST.ERROR_COLOR)(`${res.statusMessage} - CDN ${res.statusCode}`))
          process.exit(0)
          // return reject(new Error(`${res.statusMessage} - CDN`))
        }
      })
      .on('error', reject)
      .pipe(
...
let createSymlinkLevelsPromise = (src, dest, level = 0) => {
  return new Promise((resolve, reject) => {
    if (src.split('/').length <= level + 2) {
      ...
      console.log('““““on crée le répertoire', src.split('/').slice(0, 2 - src.split('/').length + level).join('/'))
