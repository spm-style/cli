...
let findPackagePromise = (generate) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    while (generate.path !== CONST.USER_DIRECTORY) {
      if (Fs.existsSync(`${generate.path}${CONST.SEPARATOR}package-spm.json`)) {
        generate.pathPackage = `${generate.path}${CONST.SEPARATOR}package-spm.json`
        if (!Fs.existsSync(`${generate.path}${CONST.SEPARATOR}spm_modules`)) {
          return reject(new Error('please install a module before using it'))
        } else {
          generate.pathModules = `${generate.path}${CONST.SEPARATOR}spm_modules`
        }
        if (Fs.existsSync(`${generate.path}${CONST.SEPARATOR}variables-spm.scss`)) {
          generate.pathVariables = `${generate.path}${CONST.SEPARATOR}variables-spm.scss`
        } else {
          return reject(new Error('no variables-spm.scss file found - '))
        }
        return resolve(generate)
      } else if (Fs.existsSync(`${generate.path}${CONST.SEPARATOR}spm_modules`)) {
        generate.pathModules = `${generate.path}${CONST.SEPARATOR}spm_modules`
        return resolve(generate)
      }
      generate.path = generate.path.substring(0, generate.path.lastIndexOf(CONST.SEPARATOR))
    }
...
let listAllClassesPromise = (generate, module) => {
  if (generate.debug) { Debug() }
  return new Promise((resolve, reject) => {
    Common.getPackageSpmFilePromise(`${generate.pathModules}${CONST.SEPARATOR}${module}${CONST.SEPARATOR}package-spm.json`)
    .then(jsonFile => {
...
let selectClassPromise = (generate) => {
  ...
  return new Promise((resolve, reject) => {
    if (!generate.classTarget) {
      Common.getPackageSpmFilePromise(`${generate.pathModules}${CONST.SEPARATOR}${generate.moduleName}${CONST.SEPARATOR}package-spm.json`)
      .then(res => {
...
let checkInstanceAvailablePromise = (generate) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    generate.pathInstance = `${generate.pathModules}${CONST.SEPARATOR}${generate.moduleName}${CONST.SEPARATOR}dist${CONST.SEPARATOR}${generate.nickname}.${generate.style}`
    if (generate.isForce) {
...
let distCreationPromise = (generate) => {
  if (generate.debug) { Debug() }
  return new Promise((resolve, reject) => {
    Fs.mkdir(`${generate.pathModules}${CONST.SEPARATOR}${generate.moduleName}${CONST.SEPARATOR}dist`, err => {
...
let instanceCreationPromise = (generate) => {
  if (generate.debug) { Debug() }
  return new Promise((resolve, reject) => {
    Fs.readFile(`${generate.pathModules}${CONST.SEPARATOR}${generate.moduleName}${CONST.SEPARATOR}${generate.jsonFile.entry}`, 'utf8', (err, data) => {
      try {
        if (err) { return reject(err) }
        let parameters = ''
        ...
        if (generate.style === 'css') { Common.createFolderIfUnexistantSync(`${generate.pathModules}${CONST.SEPARATOR}${generate.moduleName}${CONST.SEPARATOR}dist${CONST.SEPARATOR}src`) }
        ...
        Fs.writeFile(`${generate.pathModules}${CONST.SEPARATOR}${generate.moduleName}${CONST.SEPARATOR}dist${CONST.SEPARATOR}${generate.nickname}.scss`, output, err => {
          if (err) { return reject(err) }
          if (generate.style === 'css') {
            Common.convertScssToCss(generate, `${generate.pathModules}${CONST.SEPARATOR}${generate.moduleName}${CONST.SEPARATOR}dist${CONST.SEPARATOR}`, generate.nickname)
            .then(res => {
