  ...
  return new Promise((resolve, reject) => {
    ...
    let questions = [
      ...
      },
      {
        name: 'variables',
        when: (current) => {
          return current.style === 'scss' // root variables in css to be added
        },
...
let initMainCheck = (packageSpm) => { // (3a)
  return new Promise((resolve, reject) => {
    if (!Fs.existsSync(packageSpm.entry)) {
      ...
      packageSpm.warnings.push(`${packageSpm.entry} not created, make sure you import spm variables-spm.scss file in your project`)
      return resolve(packageSpm)
    }
  })
}
  ...
  return new Promise((resolve, reject) => {
    if (packageSpm.style === 'scss') {
      if (!Fs.existsSync('variables-spm.scss')) {
        let content = ''
        for (let instanceVariable of packageSpm.variables) { content = `${content}// $_${instanceVariable}: ;\n` }
        Common.writeContent(content, 'variables-spm.scss')
        .then(() => { packageSpm.successes.push('variables-spm.scss has been successfully created'); return resolve(packageSpm) })
        .catch(reject)
      } else {
        packageSpm.warnings.push('variables-spm.scss file already in your project')
        return resolve(packageSpm)
      }
    } else {
      return resolve(packageSpm)
