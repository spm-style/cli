  ...
  return new Promise((resolve, reject) => {
    Fs.readFile(file, 'utf8', function (err, data) {
      ...
      while ((i = data.indexOf('{', startIndex)) >= 0) {
        ...
        if (j < 0 || i < j) {
          if (!parseSelectors(data.substring(startIndex, i), tmpClasses) && Common.cleanValue(data.substring(startIndex, i)) !== ':root') {
            return reject(new Error(`incorrect selector found : ${data.substring(startIndex, i)} dans ${file}`))
          }
          i = data.indexOf('}', i)
        }
        startIndex = i + 1
...
let mapDependenciesClassesPromise = (publication) => {
  if (debug) console.log('mapDependenciesClassesPromise')
  return new Promise((resolve, reject) => {
...
let expressionMatchesClass = (str, customClass) => {
  for (let character of [' ', '\n', '\t', '.', '[', '{', '&', ':', ',', ';']) {
    if (str.indexOf(`.${customClass}${character}`) === 0) {
      return true
    }
  }
  return false
...
let recursiveDirectoryCopy = (path, directory, ignores, publication) => {
  if (debug) console.log('  + stage 5cI +', directory)
  return new Promise((resolve, reject) => {
            ...
            if (!ignores.includes(file) && !ignores.includes(file + '/')) {
              if (Fs.lstatSync(directory + file).isDirectory()) {
                ...
                Fs.readFile(directory + file, 'utf8', (err, data) => {
                  ...
                  let i, j, k
                  let startIndex = 0
                  i = data.indexOf('@include spm')
                  if (i >= 0) {
                    j = data.indexOf('@include spm', i + 1)
                    k = data.indexOf(`'.');`)
                    data = `${data.substring(0, k < j || j === -1 ? i : j)}${data.substring(k + 5)}`
                    startIndex = data.length
                  }
                  while ((i = data.indexOf('.', startIndex)) >= 0) {
                    for (let moduleClass in publication.dependenciesClassesMapping) {
                      if (expressionMatchesClass(data.substring(i), moduleClass)) {
                        data = `${data.substring(0, i)}%${moduleClass}${data.substring(i + 1 + moduleClass.length)}`
                        break
                      }
                    }
                    startIndex = i + 1
                  ...
                  promises.push(new Promise((resolve, reject) => {
                    Fs.writeFile(`${path}/tmp${directory.substring(path.length)}${file.endsWith('.css') ? file.substring(0, file.lastIndexOf('.')) + '.scss' : file}`, data, err => {
                      if (err) { return reject(err) }
                      return resolve()
                    })
                  }))
...
module.exports = (Program) => {
  return new Promise((resolve, reject) => {
    ...
    .action((pkg, options) => {
      let publication = new Publish(pkg, options.tag)
