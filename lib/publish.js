...
let existsPackagePromise = (publication) => { // (1a)
  ...
  return new Promise((resolve, reject) => {
    ...
    return reject('the module you\'re trying to publish has no correct package-spm.json file')
  })
}
...
let checkModuleVersion = (jsonFile, publication) => { // (1bI1)
  return new Promise((resolve, reject) => {
    if (publication.getKey('version')) {
      ...
      return reject('Incorrect version in package-spm.json')
    } else {
      Inquirer.prompt([{
        name: 'version',
      ...
      .then(answer => {
  if (!answer.version) { return reject('Aborted by user') }
...
let promptCorrection = (key, jsonFile, message) => { // (1bI2)
  return new Promise((resolve, reject) => {
    if (!jsonFile[key]) {
      ...
      .then(answer => {
  if (!answer[key]) { return reject('Aborted by user') }
...
let checkPackagePromise = (jsonFile, publication) => { // (1bI)
    ...
    if (jsonFile) {
      ...
      switch (jsonFile.style) {
        ...
        case 'less':
          ...
          break
        default:
          return reject('the project\'s style is uncorrect')
          break
      }
      // type
      switch (jsonFile.type) {
        ...
        case 'template':
          ...
          break
        default:
          return reject('the project\'s type is uncorrect')
          break
      }
      if (publication.getKey('version')) {
        jsonFile.version == publication.getKey('version')
      }
      // très basique (voir totalement faux), mais fonctionnel pour les tests
  ...
  return resolve(publication)
      ...
      .catch(err => { return reject(`the package-spm.json is not correct :\n${err}`) })
    } else {
      return reject('the package-spm.json is not correct')
    }
  })
}
...
let verifyRefDomPromise = (publication) => {
  ...
  return new Promise((resolve, reject) => {
    if (!Fs.existsSync(`${publication.getKey('path')}/ref-dom.html`)) {
      return reject('error - reference DOM missing')
    }
    Fs.readFile(`${publication.getKey('path')}/ref-dom.html`, 'utf8', (err, data) => {
      if (err) { return reject('error - issue reading reference DOM') }
...
let confirmationPublishPromise = (publication) => { // (3)
  ...
  return new Promise((resolve, reject) => {
    console.log(
      'You are about to publish the module ' + publication.getKey('name') + '@' + publication.getKey('version') + '\n' +
      'if you have the rights to publish, your contribution will be added in spm registry'
    ) // ajouter l'arborescence du projet
...
let promptUserPromise = (publication) => { // (4)
  ...
  return resolve(publication)
      ...
      .catch(err => { return reject('Prompt error - contact those lazy stu asses') })
  })
}
...
// utiliser Inquirer pour confirmer par checkbox les modules à ignorer
        ...
        while ((i = data.indexOf('{', startIndex)) >= 0 &&
          ...
          if (i >= 0 && i <= j && i <= k) {
            if (!parseSelectors(data.substring(startIndex, i), tmpClasses)) {
              return reject(`incorrect selector found : ${data.substring(startIndex, i)}`)
            }
            startIndex = i + 1
...
let recursiveCheckModule = (path, file, publication) => { // (5bI)
  ...
  return new Promise((resolve, reject) => {
    if (!Fs.existsSync(path + file)) {
      console.log('the file doesn\'t exist', path, file) // faire la liste de ces fichiers dans le warning
      return resolve(publication)
    } else {
      // fonction séparée
      Fs.readFile(path + file, 'utf8', (err, data) => {
        if (err) {
          console.log('issues in file parsing')
          return reject(publication)
        } else {
              ...
              if (!error) {
                  Fs.writeFile(publication.path + '/.tmp_sass/full.css', result.css, function (err) {
                    if (!err) {
                      ...
                      .catch(reject)
                    } else                      { return reject('error while converting scss file to css') }
                  })
  ...
  return new Promise((resolve, reject) => {
    ...
    for (let dependency in dependencies) {
      ...
      for (let ressource of publication.ressources) {
        // ajouter vérification que c'est par le main que la dépendance est utilisée
      ...
      if (!flag) {
        // WARNING MESSAGE
        // console.log('warning this dependancy is not used', dependency);
...
let sendPublicationToRegistryPromise = (publication) => { // (5f)
  ...
  return new Promise((resolve, reject) => {
    ...
    request.put({url: CONST.PUBLISH_URL, formData: formData}, function (error, response, body) {
      ...
      .then(() => {
  if (error) { return reject(`there was an error sending data to spm registry - please try again later or contact our support\n${error}`) }
