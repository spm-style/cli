  ...
  return new Promise((resolve, reject) => {
    ...
    while (currentDirectory !== CONST.USER_DIRECTORY) {
      if (Fs.existsSync(`${currentDirectory}${CONST.SEPARATOR}package-spm.json`)) {
        let jsonFile = Common.parseJsonFileSync(`${currentDirectory}${CONST.SEPARATOR}package-spm.json`)
...
let correctDevicesInProject = (responsiveness) => {
  let devices = ['watch', 'mobile', 'phablet', 'tablet', 'laptop', 'screenXl']
...
let verifyRefDomPromise = (publication) => {
  if (publication.debug) { Debug() }
  return new Promise((resolve, reject) => {
    Fs.readFile(`${publication.path}${CONST.SEPARATOR}ref-dom.html`, 'utf8', (err, data) => {
...
let updateDefaultIgnoreFiles = (ignores, publication) => {
  if (publication.debug) { Debug() }
  let defaultIgnoreFiles = ['.tmp_sass', `tmp${CONST.SEPARATOR}`, '.gitignore', '.spmignore', 'package-spm.json', 'ref-dom.html']
...
let verifyIgnoredFilesPromise = (publication) => {
  ...
  return new Promise((resolve, reject) => {
    let ignores = parseIgnoreFiles([
      `${publication.path}${CONST.SEPARATOR}.gitignore`,
      `${publication.path}${CONST.SEPARATOR}.spmignore`
    ], publication)
...
let recursiveCheckModule = (path, file, publication) => {
  if (publication.debug) { Debug() }
  if (!path.endsWith(CONST.SEPARATOR)) { path += CONST.SEPARATOR }
          ...
          .then(res => {
            return new Promise((resolve, reject) => {
              ...
              for (let module of dataAtClose) {
                promises.push(recursiveCheckModule(path + module.substring(0, module.lastIndexOf(CONST.SEPARATOR)), module.split(CONST.SEPARATOR)[module.split(CONST.SEPARATOR).length - 1], publication))
              }
...
let scssToCssForCheck = (publication) => {
  ...
  return new Promise((resolve, reject) => {
    if (publication.style === 'scss') {
      Fs.mkdir(`${publication.path}${CONST.SEPARATOR}.tmp_sass`, err => {
        if (err && err.code !== 'EEXIST') { return reject(err) }
        Fs.writeFile(`${publication.path}${CONST.SEPARATOR}.tmp_sass${CONST.SEPARATOR}entry.scss`, `@import '../tmp/${publication.entry}';`, err => {
          if (err) { return reject(err) }
          Sass.render({
            file: `${publication.path}${CONST.SEPARATOR}.tmp_sass${CONST.SEPARATOR}entry.scss`,
            outFile: `${publication.path}${CONST.SEPARATOR}.tmp_sass${CONST.SEPARATOR}full.css`
          }, function (error, result) {
            if (!error) {
              Fs.writeFile(publication.path + `${CONST.SEPARATOR}.tmp_sass${CONST.SEPARATOR}full.css`, result.css, function (err) {
                if (!err) {
                  checkClass(publication.path + `${CONST.SEPARATOR}.tmp_sass${CONST.SEPARATOR}full.css`, publication, 'css')
                  .then(() => {
                    Common.deleteFolderRecursive(`${publication.path}${CONST.SEPARATOR}.tmp_sass`, () => {
                      return resolve(publication)
                    })
                  })
                  .catch(reject)
  ...
  return new Promise((resolve, reject) => {
    ...
    for (let dependency in dependencies) {
      ...
      for (let ressource of publication.ressources) {
        if (Path.relative(`${publication.path}${CONST.SEPARATOR}spm_modules${CONST.SEPARATOR}${dependency}`, ressource.substring(0, ressource.lastIndexOf(CONST.SEPARATOR))) === '') {
          flag = true
          break
        }
      }
...
let mapDependenciesClassesPromise = (publication) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    for (let dependency in publication.dependencies) {
      promises.push(Common.getPackageSpmFilePromise(`${publication.path}${CONST.SEPARATOR}spm_modules${CONST.SEPARATOR}${dependency}${CONST.SEPARATOR}package-spm.json`))
...
let clearspmModulesTmpPromise = (publication) => {
  if (publication.debug) { Debug() }
  return new Promise((resolve, reject) => {
    Common.deleteFolderRecursivePromise(`${publication.path}${CONST.SEPARATOR}tmp${CONST.SEPARATOR}spm_modules`)
    .then(() => { return resolve(publication) })
    .catch(reject)
  })
}
...
let recursiveDirectoryCopy = (path, directory, ignores, publication) => {
  if (publication.debug) { Debug() }
  return new Promise((resolve, reject) => {
    Fs.mkdir(`${path}${CONST.SEPARATOR}tmp${directory.substring(path.length)}`, err => {
      if (err && err.code !== 'EEXIST') { return reject(err) }
          ...
          for (let file of files) {
            if (!ignores.includes(file) && !ignores.includes(file + '/')) {
              if (Fs.lstatSync(directory + file).isDirectory()) {
                if (!file.endsWith(CONST.SEPARATOR)) { file += CONST.SEPARATOR }
                let subIgnores = []
                ...
                Fs.readFile(directory + file, 'utf8', (err, data) => {
                  ...
                  promises.push(new Promise((resolve, reject) => {
                    Fs.writeFile(`${path}${CONST.SEPARATOR}tmp${directory.substring(path.length)}${file}`, data, err => {
                      if (err) { return reject(err) }
                      return resolve()
                    })
                  }))
...
let publicationCopyPromise = (publication) => {
  if (publication.debug) { Debug() }
  return new Promise((resolve, reject) => {
    Fs.mkdir(`${publication.path}${CONST.SEPARATOR}tmp`, err => {
      if (err && err.code !== 'EEXIST') { return reject(err) }
      recursiveDirectoryCopy(publication.path, `${publication.path}${CONST.SEPARATOR}`, publication.ignore, publication)
      .then(() => { return resolve(publication) })
      .catch(reject)
    })
  })
...
let createTgzPromise = (publication) => {
  ...
  return new Promise((resolve, reject) => {
    Fs.mkdir(`${publication.path}${CONST.SEPARATOR}.tmp`, err => {
      if (err && err.code !== 'EEXIST') { return reject(err) }
      Common.tgzFilePromise(`${publication.path}${CONST.SEPARATOR}tmp`, `${publication.path}${CONST.SEPARATOR}.tmp${CONST.SEPARATOR}${publication.name}.tgz`)
      .then(() => {
        return resolve(publication)
      })
      .catch(reject)
...
let cleanUpWorkingDirectory = (publication) => {
  if (publication.debug) { Debug() }
  return new Promise((resolve, reject) => {
    Common.deleteFolderRecursive(`${publication.path}${CONST.SEPARATOR}.sass-cache`, () => {
      Common.deleteFolderRecursive(`${publication.path}${CONST.SEPARATOR}tmp`, () => {
        // error management ?
        Common.deleteFolderRecursive(`${publication.path}${CONST.SEPARATOR}.tmp`, () => {
          // error management ?
          return resolve(publication)
...
let sendPublicationToRegistryPromise = (publication) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    formData.module = Fs.createReadStream(`${publication.path}${CONST.SEPARATOR}.tmp${CONST.SEPARATOR}${publication.name}.tgz`)
