...
let verifyRefDomPromise = (publication) => {
  ...
  return new Promise((resolve, reject) => {
    if (!Fs.existsSync(`${publication.path}/ref-dom.html`)) {
...
let publicationCopyPromise = (publication) => {
  ...
  return new Promise((resolve, reject) => {
    if (!Fs.existsSync(publication.path + '/tmp')) {
      Fs.mkdirSync(publication.path + '/tmp')
    }
...
let createTargzPromise = (publication) => { // (5d)
  ...
  return new Promise((resolve, reject) => {
    let read = Targz().createReadStream(publication.path + '/tmp')
    if (!Fs.existsSync(publication.path + '/.tmp')) {
      Fs.mkdirSync(publication.path + '/.tmp')
    }
    let write = Fs.createWriteStream(publication.path + '/.tmp/' + publication.name + '.tar.gz')
...
let sendPublicationToRegistryPromise = (publication) => { // (5f)
  ...
  return new Promise((resolve, reject) => {
    ...
    formData.module = Fs.createReadStream(publication.path + '/.tmp/' + publication.name + '.tar.gz')
