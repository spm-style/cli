  ...
  return new Promise((resolve, reject) => {
    ...
    while (currentDirectory !== CONST.USER_DIRECTORY) {
      if (Fs.existsSync(currentDirectory + '/package-spm.json')) {
        ls.pathPackage = currentDirectory + '/package-spm.json'
...
let parsePackageSpmRecursivePromise = (ls, table, index = 0, promises = []) => {
  return new Promise((resolve, reject) => {
    if (index >= table.length) {
      ...
      lsBis.current.path = `${ls.current.path}/spm_modules/${table[index].name}`
      Common.getPackageSpmFilePromise(`${ls.current.path}/package-spm.json`)
      .then(json => {
